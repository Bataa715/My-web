...
export default function LanguageDojoPage({ params }: { params: { langId: string } }) {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const chaptersRef = collection(firestore, `users/${user.uid}/languages/${langId}/chapters`);
            const notesRef = collection(firestore, `users/${user.uid}/languages/${langId}/notes`);
            ...
            if (langSnap.exists()) {
                ...
                toast({ title: "Алдаа", description: "Хэл олдсонгүй.", variant: "destructive" });
            }

            const chaptersData = chaptersSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Chapter));
            setChapters(chaptersData);

            const notesData = notesSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Note));
            setNotes(notesData);
             ...
             <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mt-6">
                ...
                <aside className="lg:col-span-1 space-y-4">
                    <h2 className="text-2xl font-bold text-primary">{language.name} Workspace</h2>
                     <Card>
                        <CardHeader><CardTitle>Бүлгүүд</CardTitle></CardHeader>
                        <CardContent>
                           <ul>
                                {chapters.map(chapter => (
                                    <li key={chapter.id} className="py-2 border-b border-border last:border-b-0">{chapter.title}</li>
                                ))}
                                {chapters.length === 0 && <p className="text-muted-foreground text-sm">Бүлэг байхгүй байна.</p>}
                           </ul>
                        </CardContent>
                    </Card>
                </aside>
                ...
                <main className="lg:col-span-3 space-y-6">
                    <Card>
                        ...
                        <CardContent>
                             <p className="text-muted-foreground">Сүүлийн үеийн тэмдэглэлүүд энд харагдана.</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Card>
                            ...
                            <CardContent>
                                <p className="text-muted-foreground mb-4">Өөрийгөө сорьж, мэдлэгээ бататгаарай.</p>
                         ...
                         <Card>
                            ...
                            <CardContent>
                                <p className="text-muted-foreground">Түгээмэл ашигладаг кодын хэсгүүд.</p>
