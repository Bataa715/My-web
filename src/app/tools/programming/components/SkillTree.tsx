    ...
    <Card className="glassmorphism-card neon-glow h-full">
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>Your programming learning path.</CardDescription>
        </div>
        {isEditMode && (
      ...
      <CardContent className="space-y-4">
        {concepts.map((concept) => (
          <Card key={concept.id} className="flex flex-col relative group bg-background/50 border-primary/20">
             {isEditMode && (
              <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-7 w-7 hover:bg-destructive/10 text-muted-foreground hover:text-destructive"><Trash2 className="h-4 w-4"/></Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent>
                    <AlertDialogHeader>
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-md font-medium text-accent">{concept.title}</CardTitle>
         ...
         {concepts.length === 0 && (
          <div className="text-center text-muted-foreground py-10 md:col-span-2 lg:col-span-3">
             {isEditMode ? "Unlock new skills by adding concepts." : "Skill tree is empty."}
          </div>
        )}
      </CardContent>
    </Card>
