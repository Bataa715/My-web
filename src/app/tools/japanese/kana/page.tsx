    ...
    useEffect(() => {
        const getKanaData = async () => {
            ...
            try {
                ...
                const hiraganaData = hiraganaSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Kana));
                setHiragana(hiraganaData);

                const katakanaSnap = await getDocs(collection(firestore, "katakana"));
                const katakanaData = katakanaSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Kana));
                 ...
                 {loading ? (
                    ...
                    <>
                        <TabsContent value="hiragana">
                           <KanaGrid kana={hiragana} title="Хирагана үсгүүд" collectionPath='hiragana' />
                        </TabsContent>
                        <TabsContent value="katakana">
                           <KanaGrid kana={katakana} title="Катакана үсгүүд" collectionPath='katakana' />
                        </TabsContent>
                    </>
                )}
            </Tabs>
