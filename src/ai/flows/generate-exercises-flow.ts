...
const MCQSchema = z.object({
  ...
  options: z.array(z.string()).min(3).max(4).describe('An array of 3 to 4 possible answers.'),
...
const WritingTaskSchema = z.object({
  instruction: z.string().describe('The instruction for the user (e.g., "Translate this sentence", "Rewrite this sentence in the negative form").'),
...
const GenerateExercisesInputSchema = z.object({
  ...
  numWriting: z.number().int().min(0).max(5).describe('The number of writing tasks to generate.'),
});
...
const generateExercisesFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output) {
      throw new Error('Failed to generate exercises. The AI model did not return a valid output.');
    }
    return output;
  }
);
