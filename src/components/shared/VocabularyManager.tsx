 ...
 const handleDelete = async (wordId: string) => {
  ...
  const handlePracticeComplete = (memorizedIds: string[]) => {
    ...
    if (memorizedIds.length > 0 && user && firestore) {
        ...
        batch.commit()
            .then(() => {
                setWords(prevWords => prevWords.map(w => memorizedIds.includes(w.id!) ? { ...w, memorized: true } as T : w));
                toast({ title: "Амжилттай", description: `${memorizedIds.length} үг цээжилсэн төлөвт орлоо.`});
            })
            .catch(e => {
  ...
  return (
    <>
        ...
        <div className="mt-8">
            <h2 className="text-2xl font-bold text-center mb-4">Сонирхолтой аргууд</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 <Card className="hover:shadow-primary/20 hover:shadow-lg transition-shadow">
                    ...
                    <CardContent>
                        <CardDescription>Картаар эргүүлж цээжилсэн үгээ бататгах.</CardDescription>
                        <Button className="mt-4 w-full" onClick={() => setGameMode('flashcard')} disabled={filteredWords.length === 0}>
                           {filter === 'all' ? 'Бүх үгсээр' : 
                           filter === 'memorized' ? 'Цээжилсэн үгсээр' : 
                           filter === 'not-memorized' ? 'Цээжлээгүй үгсээр' : 
                           filter === 'favorite' ? 'Онцолсон үгсээр' : ''} эхлэх
                        </Button>
                    </CardContent>
                </Card>
                 <Card className="hover:shadow-primary/20 hover:shadow-lg transition-shadow">
