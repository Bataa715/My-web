  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {rules.map((rule, index) => (
        <Card key={rule.id || index} className="relative group overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col">
           ...
           <CardContent className="flex justify-end items-center mt-auto">
               <Link href={`${pathname}/${rule.id}`} className="flex items-center text-sm text-primary hover:underline">
                    Дэлгэрэнгүй
