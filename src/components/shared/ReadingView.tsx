...
export default function ReadingView({ material }: ReadingViewProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(notesQuery, snapshot => {
      ...
      setNotes(
        fetchedNotes.sort((a, b) => (a.createdAt as any) - (b.createdAt as any))
      );
    });
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Pencil className="h-5 w-5" /> Миний тэмдэглэлүүд
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {notes.length > 0 ? (
              notes.map(note => (
                ...
                >
                  <p className="font-semibold border-l-2 border-primary pl-2 italic">
                    "{note.selectedText}"
                  </p>
